...
from nia.nova.core.endpoints import get_world, get_analytics_agent, get_orchestration_agent
from nia.nova.core.auth import get_api_key
from nia.nova.core.error_handling import ResourceNotFoundError, ValidationError
...
def mock_api_key():
    ...
    from nia.nova.core.auth import API_KEYS, get_ws_api_key
    API_KEYS["test-key"] = {
...
@pytest.fixture(autouse=True)
def setup_dependencies(mock_analytics_agent, mock_orchestration_agent, world):
    """Set up dependencies for all tests."""
    from nia.nova.core.endpoints import get_analytics_agent, get_orchestration_agent, get_world
...
@pytest.mark.asyncio
@pytest.mark.timeout(30)  # Add timeout of 30 seconds
async def test_analytics_websocket(world, mock_analytics_agent, mock_orchestration_agent):
