...
@pytest.fixture
def integration_agent(mock_memory_system, mock_world):
...
@pytest.mark.asyncio
async def test_initialization(integration_agent):
    """Test agent initialization."""
    assert integration_agent.name == "TestIntegration"
...
@pytest.mark.asyncio
async def test_integrate_with_different_domains(integration_agent):
...
async def test_reflection_recording(integration_agent, mock_memory_system):
    ...
    reflection_calls = mock_memory_system.semantic.store.record_reflection.call_args_list
    assert any("Integration failed" in str(call) for call in reflection_calls)
...
async def test_critical_issue_reflection(integration_agent, mock_memory_system):
    ...
    reflection_calls = mock_memory_system.semantic.store.record_reflection.call_args_list
    assert any("Critical integration issues" in str(call) for call in reflection_calls)
...
async def test_important_insight_reflection(integration_agent, mock_memory_system):
    ...
    reflection_calls = mock_memory_system.semantic.store.record_reflection.call_args_list
    assert any("Important connections discovered" in str(call) for call in reflection_calls)

@pytest.mark.asyncio
async def test_emotion_updates(integration_agent):
...
@pytest.mark.asyncio
async def test_desire_updates(integration_agent):
